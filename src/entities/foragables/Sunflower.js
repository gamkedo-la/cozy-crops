...
export default class Sunflower extends Forageable {
  ...
  buildAnimations () {
    const animationKeys = Object.keys(ForageableAnimations.Sunflower)
    animationKeys.forEach((key, index) => {
      const config = Object.assign({}, ForageableAnimations.Sunflower[key])
      config.owner = this
      config.game = this.game
      config.imageManager = this.imageManager
      config.imageSrc = ForageableAnimations.Sunflower[key].spritesheet
      config.canvas = this.imageManager.getImageWithSrc(ForageableAnimations.Sunflower[key].spritesheet)
