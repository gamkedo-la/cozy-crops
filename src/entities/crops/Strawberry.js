...
export default class Strawberry extends Crop {
  ...
  buildAnimations () {
    const animationKeys = Object.keys(CropAnimations.Strawberry)
    animationKeys.forEach((key, index) => {
      const config = Object.assign({}, CropAnimations.Strawberry[key])
      config.owner = this
      config.game = this.game
      config.imageManager = this.imageManager
      config.imageSrc = CropAnimations.Strawberry[key].spritesheet
      config.canvas = this.imageManager.getImageWithSrc(CropAnimations.Strawberry[key].spritesheet)
